...
import { useListings, useVendors } from '@/hooks';
...
export function Profile() {
  ...
  useEffect(() => {
    ...
    // If viewing own profile (/profile without ID)
    if (!id) {
      if (!isAuthenticated) {
      ...
      // Wait for currentUser to be populated even after auth is done
  ...
  const loadOtherUserProfile = async (userId: string) => {
    try {
      ...
      console.error('Error in loadOtherUserProfile:', err);
      ...
      <Layout>
        <div className="max-w-7xl mx-auto px-4 py-12 text-center">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-6 max-w-md mx-auto">
            <h3 className="text-lg font-medium text-yellow-800 mb-2">Profile Not Found</h3>
            <p className="text-yellow-700 mb-4">We couldn't find your profile data. This might happen if your account was just created.</p>
            <Button 
